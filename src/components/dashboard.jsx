  ...
  return (
    <div className="bg-white min-h-screen font-sans text-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 flex flex-wrap gap-4 md:gap-8 items-center border-b border-slate-50 pb-8">
        {['All', 'Outerwear', 'Pants', 'Basics', 'Shirts'].map(cat => (
          <button 
            ...
            className={`text-xs uppercase tracking-widest font-bold transition-all ${activeCategory === cat ? 'text-black border-b-2 border-black' : 'text-slate-400 hover:text-black'}`}
          >
            {cat}
          </button>
        ))}
        ...
        <AnimatePresence mode="popLayout">
          {filteredProducts.map(product => (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={product.id} className="group">
              <div className="relative aspect-[3/4] overflow-hidden bg-slate-100 rounded-sm cursor-crosshair" onClick={() => setSelectedProduct(product)}>
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <span className="bg-white text-black px-6 py-3 text-[10px] font-bold uppercase tracking-tighter">View Details</span>
                </div>
              </div>
      ...
      <AnimatePresence>
        {isCartOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsCartOpen(false)} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100]" />
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-[110] shadow-2xl p-8 flex flex-col">
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-2xl font-black uppercase tracking-tighter italic underline decoration-red-500">Your Bag</h2>
                <X className="cursor-pointer hover:rotate-90 transition" onClick={() => setIsCartOpen(false)} />
              </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="mt-4 uppercase text-[10px] tracking-widest font-bold">Bag is Empty</p>
                  </div>
                ) : (
                  cart.map((item, i) => (
                    <div key={i} className="flex gap-4 mb-8 border-b border-slate-50 pb-8">
                      <img src={item.img} className="w-20 h-24 object-cover rounded-sm shadow-md" />
      ...
      <AnimatePresence>
        {selectedProduct && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white w-full max-w-4xl grid md:grid-cols-2 shadow-2xl overflow-hidden rounded-sm">
              <button className="absolute top-4 right-4 z-20" onClick={() => setSelectedProduct(null)}><X /></button>
              <img src={selectedProduct.img} className="w-full h-[500px] object-cover" />
              <div className="p-12 flex flex-col justify-center">
                <h2 className="text-4xl font-black italic tracking-tighter mb-4 uppercase">{selectedProduct.name}</h2>
                <p className="text-slate-500 mb-8 leading-relaxed">{selectedProduct.desc}</p>
                <div className="mb-8">
                  <label className="text-[10px] font-black uppercase tracking-widest block mb-4">Select Size</label>
                  <div className="flex gap-4">
                    {['S', 'M', 'L'].map(s => (
                      <button key={s} onClick={() => setSelectedSize(s)} className={`w-12 h-12 border transition-all flex items-center justify-center text-xs font-bold ${selectedSize === s ? 'bg-black text-white border-black' : 'border-slate-200 hover:border-black'}`}>{s}</button>
                    ))}
                  </div>
                </div>
